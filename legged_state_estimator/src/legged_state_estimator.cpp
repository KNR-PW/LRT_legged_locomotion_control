...
LeggedStateEstimator::LeggedStateEstimator(const LeggedStateEstimatorSettings& settings)
  ...
  const ocs2::scalar_t contact_position_cov = settings.contact_position_noise * settings.contact_position_noise;
  const ocs2::scalar_t contact_rotation_cov = settings.contact_rotation_noise * settings.contact_rotation_noise;
  Matrix6d cov_leg = Matrix6d::Zero();
  cov_leg.topLeftCorner<3, 3>() = contact_position_cov * ocs2::matrix3_t::Identity();
  cov_leg.bottomRightCorner<3, 3>() = contact_rotation_cov * ocs2::matrix3_t::Identity();
...
void LeggedStateEstimator::init(const ocs2::vector3_t& base_pos,
                                const ocs2::vector4_t& base_quat,
                                const ocs2::vector3_t& base_lin_vel_world,
                                const ocs2::vector3_t& imu_gyro_bias,
                                const ocs2::vector3_t& imu_lin_accel_bias) {
...
void LeggedStateEstimator::init(const ocs2::vector3_t& base_pos,
  ...
  robot_model_.updateKinematics(ocs2::vector3_t::Zero(), base_quat, qJ);
  ocs2::scalar_t base_height = 0;
  ...
  const ocs2::vector3_t base_pos_correct 
...
void LeggedStateEstimator::update(const ocs2::vector3_t& imu_gyro_raw, 
  ...
  for(int  i = 0;  i < robot_model_.numContacts(); ++i) {
    ...
    const ocs2::scalar_t contact_force_cov = contact_estimator_.getContactForceCovariance()[i];
    leg_kinematics_[i].setContactPositionCovariance(
        contact_force_cov*ocs2::matrix3_t::Identity());
  }
...
const ocs2::vector3_t& LeggedStateEstimator::getBasePositionEstimate() const {
  return base_pos_estimate_;
}


const ocs2::matrix3_t& LeggedStateEstimator::getBaseRotationEstimate() const {
  return base_rot_estimate_;
}


const ocs2::vector4_t& LeggedStateEstimator::getBaseQuaternionEstimate() const {
  return base_quat_estimate_;
}


const ocs2::vector3_t& LeggedStateEstimator::getBaseLinearVelocityEstimateWorld() const {
  return base_lin_vel_world_estimate_;
}


const ocs2::vector3_t LeggedStateEstimator::getBaseLinearVelocityEstimateLocal() const {
  return base_lin_vel_local_estimate_;
}


const ocs2::vector3_t& LeggedStateEstimator::getBaseAngularVelocityEstimateWorld() const {
  return base_ang_vel_world_estimate_;
}


const ocs2::vector3_t& LeggedStateEstimator::getBaseAngularVelocityEstimateLocal() const {
  return base_ang_vel_local_estimate_;
}


const ocs2::vector3_t& LeggedStateEstimator::getIMUGyroBiasEstimate() const {
  return imu_gyro_bias_estimate_;
}


const ocs2::vector3_t& LeggedStateEstimator::getIMULinearAccelerationBiasEstimate() const {
  return imu_lin_acc_bias_estimate_;
}


const ocs2::vector_t& LeggedStateEstimator::getJointVelocityEstimate() const {
  return lpf_dqJ_.getEstimate();
}


const ocs2::vector_t& LeggedStateEstimator::getJointAccelerationEstimate() const {
  return lpf_ddqJ_.getEstimate();
}


const ocs2::vector_t& LeggedStateEstimator::getJointTorqueEstimate() const {
