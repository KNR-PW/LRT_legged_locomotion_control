...
ocs2::matrix3_t skew(const ocs2::vector3_t& v) {
  ...
  ocs2::matrix3_t M = ocs2::matrix3_t::Zero();
...
ocs2::matrix3_t Gamma_SO3(const ocs2::vector3_t& w, const int m, 
  ...
  if(theta < exp_map_tol) {
      return (1.0/factorial(m))*ocs2::matrix3_t::Identity(); // TODO: There is a better small value approximation for exp() given in Trawny p.19
  } 
  const ocs2::matrix3_t A = skew(w);
  const ocs2::scalar_t theta2 =  theta*theta;

  // Closed form solution for the first 3 cases
  switch (m) {
    case 0: // Exp map of SO(3)
      return ocs2::matrix3_t::Identity() + (sin(theta)/theta)*A + ((1-cos(theta))/theta2)*A*A;
    
    case 1: // Left Jacobian of SO(3)
      // eye(3) - A*(1/theta^2) * (R - eye(3) - A);
      // eye(3) + (1-cos(theta))/theta^2 * A + (theta-sin(theta))/theta^3 * A^2;
      return ocs2::matrix3_t::Identity() + ((1-cos(theta))/theta2)*A + ((theta-sin(theta))/(theta2*theta))*A*A;

    case 2: 
      // 0.5*eye(3) - (1/theta^2) * (R - eye(3) - A - 0.5*A^2);
      // 0.5*eye(3) + (theta-sin(theta))/theta^3 * A + (2*(cos(theta)-1) + theta^2)/(2*theta^4) * A^2
      return 0.5*ocs2::matrix3_t::Identity() + (theta-sin(theta))/(theta2*theta)*A + (theta2 + 2*cos(theta)-2)/(2*theta2*theta2)*A*A;

    default: // General case 
      const ocs2::matrix3_t R = ocs2::matrix3_t::Identity() + (sin(theta)/theta)*A + ((1-cos(theta))/theta2)*A*A;
      ocs2::matrix3_t S = ocs2::matrix3_t::Identity();
      ocs2::matrix3_t Ak = ocs2::matrix3_t::Identity();
      ...
      if(m==0) { 
          ...
          return (1.0/kfactorial)*ocs2::matrix3_t::Identity() + (pow(-1,(m+1)/2)/pow(theta,m+1))*A * (R - S);
      } 
      else { // even
          return (1.0/kfactorial)*ocs2::matrix3_t::Identity() + (pow(-1,m/2)/pow(theta,m)) * (R - S);
      }
  }
}
...
ocs2::matrix_t Exp_SEK3(const ocs2::vector_t& v,  const ocs2::scalar_t exp_map_tol) {
  ...
  if(theta < exp_map_tol) {
    R = ocs2::matrix3_t::Identity();
    Jl = ocs2::matrix3_t::Identity();
  } else {
    const ocs2::matrix3_t A = skew(w);
    const ocs2::scalar_t theta2 = theta*theta;
    ...
    const ocs2::matrix3_t A2 = A*A;
    R.noalias() = ocs2::matrix3_t::Identity() 
                   + (stheta/theta) * A + oneMinusCosTheta2 * A2;
    Jl.noalias() = ocs2::matrix3_t::Identity() 
