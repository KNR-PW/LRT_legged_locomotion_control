...
InEKFState::InEKFState() : 
  X_(ocs2::matrix_t::Identity(5,5)), 
  Theta_(ocs2::matrix_t::Zero(6,1)), 
  P_(ocs2::matrix_t::Identity(15,15)) {}


// Initialize with X
InEKFState::InEKFState(const ocs2::matrix_t& X) : 
    X_(X), Theta_(ocs2::matrix_t::Zero(6,1)) {
    P_ = ocs2::matrix_t::Identity(3*this->dimX()+this->dimTheta()-6, 3*this->dimX()+this->dimTheta()-6);
}


// Initialize with X and Theta
InEKFState::InEKFState(const ocs2::matrix_t& X, const ocs2::vector_t& Theta) : 
    X_(X), Theta_(Theta) {
    P_ = ocs2::matrix_t::Identity(3*this->dimX()+this->dimTheta()-6, 3*this->dimX()+this->dimTheta()-6);
}
...
InEKFState::InEKFState(const ocs2::matrix_t& X, const ocs2::vector_t& Theta, const ocs2::matrix_t& P) : 
    X_(X), Theta_(Theta), P_(P) {}
// TODO: error checking to make sure dimensions are correct and supported


const ocs2::matrix_t& InEKFState::getX() const { return X_; }
const ocs2::vector_t& InEKFState::getTheta() const { return Theta_; }
const ocs2::matrix_t& InEKFState::getP() const { return P_; }
const Eigen::Block<const ocs2::matrix_t, 3, 3> InEKFState::getRotation() const { return X_.template block<3,3>(0,0); }
const Eigen::Block<const ocs2::matrix_t, 3, 1> InEKFState::getVelocity() const { return X_.template block<3,1>(0,3); }
const Eigen::Block<const ocs2::matrix_t, 3, 1> InEKFState::getPosition() const { return X_.template block<3,1>(0,4); }
const Eigen::Block<const ocs2::matrix_t, 3, 1> InEKFState::getVector(int index) const { return X_.template block<3,1>(0,index); }

const Eigen::VectorBlock<const ocs2::vector_t, 3> InEKFState::getGyroscopeBias() const { return Theta_.template head<3>(); }
const Eigen::VectorBlock<const ocs2::vector_t, 3> InEKFState::getAccelerometerBias() const { return Theta_.template tail<3>(3); }

const Eigen::Block<const ocs2::matrix_t, 3, 3> InEKFState::getRotationCovariance() const { return P_.template block<3,3>(0,0); }
const Eigen::Block<const ocs2::matrix_t, 3, 3> InEKFState::getVelocityCovariance() const { return P_.template block<3,3>(3,3); }
const Eigen::Block<const ocs2::matrix_t, 3, 3> InEKFState::getPositionCovariance() const { return P_.template block<3,3>(6,6); }
const Eigen::Block<const ocs2::matrix_t, 3, 3> InEKFState::getGyroscopeBiasCovariance() const { return P_.template block<3,3>(P_.rows()-6,P_.rows()-6); }
const Eigen::Block<const ocs2::matrix_t, 3, 3> InEKFState::getAccelerometerBiasCovariance() const { return P_.template block<3,3>(P_.rows()-3,P_.rows()-3); }
...
const ocs2::matrix_t InEKFState::getWorldX() const {
...
const ocs2::matrix_t InEKFState::getBodyX() const {
...
void InEKFState::copyDiagXinv(const int n, ocs2::matrix_t& BigXinv) const {
  const int dimX = this->dimX();
  ocs2::matrix_t Xinv = this->calcXinv();
  for(int  i = 0;  i < n; ++i) {
...
ocs2::matrix_t InEKFState::calcXinv() const {
  const int dimX = this->dimX();
  ocs2::matrix_t Xinv = ocs2::matrix_t::Identity(dimX,dimX);
