...
namespace legged_state_estimator 
{
  ...
  ContactEstimator::ContactEstimator(const RobotModel& robot_model,
    const ContactEstimatorSettings& settings): settings_(settings),
      contact_force_estimate_(robot_model.numContacts(), vector3_t::Zero()),
      contact_force_estimate_prev_(robot_model.numContacts(), vector3_t::Zero()),
      contact_surface_normal_(robot_model.numContacts(), vector3_t::Zero()),
  ...
  {
    if(settings.beta0.size() > MAX_LEG_NUMBER) 
    {
      throw std::invalid_argument(
        "[ContactEstimator] invalid argment: settings.beta0.size() must be smaller than " + MAX_LEG_NUMBER);
    }

    if(settings.beta0.size() < 2) 
    {
      throw std::invalid_argument(
        "[ContactEstimator] invalid argment: settings.beta0.size() must be greater than 1");
    }

    if(settings.beta1.size() > MAX_LEG_NUMBER) 
    {
      throw std::invalid_argument(
        "[ContactEstimator] invalid argment: settings.beta1.size() must be smaller than " + MAX_LEG_NUMBER);
    }

    if(settings.beta1.size() < 2) 
    {
      throw std::invalid_argument(
        "[ContactEstimator] invalid argment: settings.beta1.size() must be greater than 1");
    }
  ...
  /******************************************************************************************************/
  /******************************************************************************************************/
  ContactEstimator::ContactEstimator(): settings_(), contact_force_estimate_(), 
    contact_force_estimate_prev_(), contact_surface_normal_(), 
    normal_contact_force_estimate_(), normal_contact_force_estimate_prev_(), 
    contact_probability_(), contact_force_covariance_(), contact_state_(), 
    num_contacts_(0) { }
      ...
      const vector_t& tauJ) 
  {
    ...
    for(int  i = 0;  i < num_contacts_; ++i) 
    {
      const scalar_t df = normal_contact_force_estimate_[i] - normal_contact_force_estimate_prev_[i];
    ...
    for(int  i = 0;  i < num_contacts_; ++i) 
    {
      contact_state_.push_back(std::pair<int, bool>(i, 
        (contact_probability_[i] >= settings_.contact_probability_threshold)));
    }
  }
  ...
  /******************************************************************************************************/
  /******************************************************************************************************/
  const std::vector<scalar_t>& ContactEstimator::getContactProbability() const 
  {
  ...
  /******************************************************************************************************/
  /******************************************************************************************************/
  const std::vector<scalar_t>& ContactEstimator::getContactForceCovariance() const 
  {
  ...
  /******************************************************************************************************/
  /******************************************************************************************************/
  const std::vector<vector3_t>& ContactEstimator::getContactForceEstimate() const 
  {
  ...
  /******************************************************************************************************/
  /******************************************************************************************************/
  const std::vector<scalar_t>& ContactEstimator::getNormalContactForceEstimate() const 
  {
  ...
  /******************************************************************************************************/
  /******************************************************************************************************/
  const std::vector<vector3_t>& ContactEstimator::getContactSurfaceNormal() const 
  {
